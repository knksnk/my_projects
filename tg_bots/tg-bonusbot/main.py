...
bot = telebot.TeleBot("5554365630:AAF5HYoi-WG_cttjgmkTnloHVRB77DGIHkE")

url = 'https://crm.atformula.ru/api/telegram/dombot2/webhook/bcbafd6c823b0408afe7aaa1e649b14a'
headers = {
...
def start(message):
    ...
    url1 = 'https://crm.atformula.ru/api/telegram/dombot2/userexists/bcbafd6c823b0408afe7aaa1e649b14a/' + str(message.from_user.id) 
    r = requests.get(url1)
    r = r.json()
    flag = r["exists"]
    if flag:
    ...
    if not flag:
        ...
        bot.reply_to(message, 'Вы уже зарегестрированы в боте. Тем не менее, вы можете еще раз пройти бот и узнать много полезной информации. \n\nЯ помощник в команде обучающего курса "Формула перепланировок". А вы?', reply_markup=markup)
...
def get_text_messages(message):
    if message.text == 'Я дизайнер 🎨':
        ...
        x["request"] = "Создавать грамотные с точки зрения закона проекты"
        ...
        x["request"] = 'Увеличить свой доход в 2 раза повысив квалификацию'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Получать повышенную комиссию за рекомендацию клиента'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Узнать как распознать квартиру с подвохом и не дать себя обмануть во время сделки'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Стабильность дохода - меньше работать и больше зарабатывать'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Нашел перепланировку в своей квартире'
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        ...
        x["need"] = 'Хочу согласовать сам'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)    
        bot.send_message(message.from_user.id, "Согласование перепланировки - “увлекательный процесс”. Но самостоятельное согласование поможет сэкономить.\n\nВот мои рекомендации👇:\n- закажите проектно-техническую документацию и получите пошаговый план согласования. Для этого напишите «Заказать». \nПодробнее по <a href='https://formula-ncpr.ru/?ysclid=lnlydy4hjy317154263'>ссылке</a> \n- подпишитесь на телеграмм канал «Перепланировки с Анной Страгис», там есть и нормы перепланировок, и интересные кейсы, и прямые эфиры.", reply_markup=markup, parse_mode='HTML', disable_web_page_preview=True)

    elif message.text == 'Заказать' or message.text == 'заказать':
        bot.send_message(message.from_user.id, "Вы оставили заявку на получение проектно-технической документации. Мы обязательно с Вами свяжемся.", reply_markup=types.ReplyKeyboardRemove() )
        x["need"] = 'Хочу согласовать сам, оставил заявку'
        ans = json.dumps(x, ensure_ascii=False).encode('utf-8')
        requests.post(url, headers=headers, data=ans)
        
    elif message.text == 'Хочу получить услугу «согласование перепланировки':
        x["need"] = 'Хочу получить услугу «согласование перепланировки'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Просто было интересно'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        ...
        x["request"] = 'Понял, как нужно законно сделать ремонт'
        markup = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton("«Перепланировки с Анной Страгис»", url='https://t.me/formula_pereplanirovok')
        markup.add(button1)           
        bot.send_message(message.from_user.id, "Отлично🔥, спасибо за обратную связь, если остались вопросы, свяжитесь с нами +7 913 970-00-37.\n\nМы поможем согласовать перепланировку любой сложности.\n\n- если вам интересная сфера перепланировок, пройдите наш обучающий курс.\nПодробнее по <a href='https://formula-ncpr.ru/kurs'>ссылке</a> \n- подпишитесь на телеграмм канал «Перепланировки с Анной Страгис», там есть и нормы перепланировок, и интересные кейсы, и прямые эфиры.", reply_markup=markup, parse_mode='HTML', disable_web_page_preview=True)
...
def get_name(message):
    ...
    if not flag:
        ...
        bot.reply_to(message, "Приятно познакомиться, " + message.text + "!\n\nНажмите кнопку ниже \U00002B07, тем самым указав Ваш номер телефона для связи 📞.", reply_markup=keyboard)
